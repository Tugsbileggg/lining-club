...
const PAYMENT_OPTIONS = [
  { v: "qpay", label: "QPay", desc: "Аюулгүй төлбөрийн хуудсаар QR уншуулж төлнө" },
  { v: "manual", label: "Дансаар / Бэлэн", desc: "Захиалга баталгаажсаны дараа холбогдоно" },
] as const;

/**
 * `qpayEnabled` comes from the server — QPay is offered only when the Wire
 * connection is configured, so a half-configured deployment never shows a
 * payment method it cannot fulfil.
 */
export function CheckoutForm({ qpayEnabled }: { qpayEnabled: boolean }) {
  ...
  const options = qpayEnabled
    ? PAYMENT_OPTIONS
    : PAYMENT_OPTIONS.filter((o) => o.v !== "qpay");
  ...
  const {
    ...
    defaultValues: { paymentMethod: qpayEnabled ? "qpay" : "manual" },
  });
  const paymentMethod = watch("paymentMethod");

  async function onSubmit(values: CheckoutInput) {
    // The order is persisted to Firestore and a confirmation email is sent
    // server-side (console mode until Resend is configured). QPay orders then
    // continue to the QR page, where the Wire invoice is created.
    try {
      ...
      // The payment token is kept in sessionStorage rather than the URL so it
      // never lands in browser history, server logs or a referrer header.
      if (values.paymentMethod === "qpay" && order.paymentToken) {
        sessionStorage.setItem(
          paymentTokenKey(order.orderNumber),
          order.paymentToken,
        );
        clear();
        router.push(`/checkout/pay/${encodeURIComponent(order.orderNumber)}`);
        return;
      }
